package digital.euforia.app.ui.programs.material
...
private fun LazyListScope.similarItem() = item(key = "similar") {
    val localizedRes = LocalLocalizedRes.current
    Column(
        modifier = Modifier
            .fillMaxWidth(),
...
private fun handleSideEffect(sideEffect: PublicationSideEffect) {
    when (sideEffect) {
        else -> {}
    }
}