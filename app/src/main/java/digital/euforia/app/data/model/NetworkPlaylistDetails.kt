...
data class NetworkPlaylistDetails(
    ...
    @field:Json(name = "scene_ids") val sceneIds: List<Int>? = null,
    @field:Json(name = "scenes") val scenes: List<NetworkScene> = emptyList(),
)

fun NetworkPlaylistDetails.toEntity(): SoundscapePlaylist = SoundscapePlaylist(
    id = id,
    alias = alias,
    name = name,
    ...
    imageUrl = imageUrl,
    pro = pro,
    sceneIds = sceneIds ?: scenes.map { it.id },
)
