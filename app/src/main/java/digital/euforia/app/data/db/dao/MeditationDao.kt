package digital.euforia.app.data.db.dao

import androidx.room.Dao
import androidx.room.Insert
import androidx.room.OnConflictStrategy
import androidx.room.Query
import digital.euforia.app.data.db.entity.MeditationEntity
import kotlinx.coroutines.flow.Flow

@Dao
interface MeditationDao {
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertAll(items: List<MeditationEntity>)

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insert(item: MeditationEntity)

    @Query("SELECT * FROM meditations WHERE id = :id")
    fun getByIdFlow(id: Int): Flow<MeditationEntity?>

    @Query("SELECT * FROM meditations WHERE main_package_id = :packageId")
    fun getByPackageIdFlow(packageId: Int): Flow<List<MeditationEntity>>

    @Query("DELETE FROM meditations")
    suspend fun clearAll()
}